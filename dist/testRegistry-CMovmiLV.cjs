...
class TestCompiler {
  ...
  constructor() {
    this.cacheDir = path.join(node_os.tmpdir(), "magnitude-cache");
    if (!fs.existsSync(this.cacheDir)) {
      fs.mkdirSync(this.cacheDir, { recursive: true });
    }
  }
  async compileFile(filePath) {
    const fileName = path.basename(filePath).replace(".ts", ".mjs");
    ...
    fs.writeFileSync(
      path.join(this.cacheDir, "package.json"),
...
class TestRegistry {
  ...
  async loadTestFile(absoluteFilePath, relativeFilePath) {
    try {
      ...
      await import(node_url.pathToFileURL(compiledPath).href);
    } catch (error) {
...
exports.knownCostMap = knownCostMap;
