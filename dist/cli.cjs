...
function findConfig(searchRoot) {
  try {
    const configFiles = glob.glob.sync("**/magnitude.config.ts", {
      cwd: searchRoot,
...
async function readConfig(configPath) {
  try {
    const compiler = new testRegistry.TestCompiler();
...
Check to evaluate:
`.trim();
class TestCaseAgent extends magnitudeCore.BrowserAgent {
...
<check>${description}</check>`;
    ...
    if (!response.passed) throw new magnitudeCore.AgentError(`Check failed: ${description}`, { variant: "check_failed" });
  }
}
...
async function sendTelemetry(state) {
  ...
  const userId = magnitudeCore.getMachineId();
  const codebaseId = magnitudeCore.getCodebaseId();
  if (codebaseId) {
    try {
      magnitudeCore.posthog.groupIdentify({
        groupType: "codebase",
        groupKey: codebaseId
        //properties: {}
      });
  ...
  try {
    ...
    await magnitudeCore.posthog.shutdown();
  } catch (error) {
...
class TestRunner {
  ...
  async run() {
    const { agentOptions, browserOptions } = magnitudeCore.buildDefaultBrowserAgentOptions({
      agentOptions: {
...
function generateSummaryString() {
  ...
  let costDescription = "";
  for (const [model, costs] of Object.entries(testRegistry.knownCostMap)) {
  ...
  onTestStateUpdated(test, newState) {
    ...
    if (!this.firstModelReportedInUI && newState.modelUsage && newState.modelUsage.length > 0) {
      ...
      if (firstModelEntry && firstModelEntry.llm) {
        modelNameToReport = testRegistry.describeModel(firstModelEntry.llm);
      }
...
async function startWebServer(config) {
  ...
  const child = require$$1.spawn(command, { shell: true, stdio: "inherit" });
...
async function initializeProject(force = false, destination = "tests/magnitude") {
  ...
  try {
    require$$1.execSync("npx playwright install chromium", { stdio: "inherit" });
...
program.name("magnitude").description("Run Magnitude test cases").argument("[filter]", "glob pattern for test files (quote if contains spaces or wildcards)").option("-w, --workers <number>", "number of parallel workers for test execution", "1").option("-p, --plain", "disable pretty output and print lines instead").option("-d, --debug", "enable debug logs").action(async (filter, options) => {
  ...
  let termAppRenderer = null;
  if (showUI) {
    termAppRenderer = new TermAppRenderer(config, tests);
  }
  const runnerConfig = {
    workerCount,
    renderer: termAppRenderer || {
      // Use TermAppRenderer or a dummy for plain/debug
